...
(define-constant err-batch-failed (err u108))
...
(define-private (distribute-to-holder (holder principal) (acc-context { asset-id: uint, total-tokens: uint, revenue-per-token: uint, current-block: uint, success: bool }))
  (let ((user-tokens (default-to u0 (map-get? asset-tokens { asset-id: (get asset-id acc-context), holder: holder })))
        (user-share (/ (* (get revenue-per-token acc-context) user-tokens) u1000000)))
    (if (and (get success acc-context) (> user-share u0))
      (match (as-contract (stx-transfer? user-share (as-contract tx-sender) holder))
        success-result (begin
          (map-set last-claim { asset-id: (get asset-id acc-context), holder: holder } (get current-block acc-context))
          acc-context
        )
        error-result (merge acc-context { success: false })
      )
      acc-context
    )
  )
...
(define-public (distribute-revenue-batch (asset-id uint))
  (let ((asset (unwrap! (map-get? assets asset-id) err-not-found))
        (holders (unwrap! (map-get? asset-holders asset-id) err-not-found))
        (revenue-available (default-to u0 (map-get? revenue-pool asset-id)))
        (total-tokens (get token-supply asset)))
    (asserts! (is-eq tx-sender (get owner asset)) err-unauthorized)
    (asserts! (get is-active asset) err-asset-not-active)
    (asserts! (> revenue-available u0) err-insufficient-balance)
    (let ((revenue-per-token (/ (* revenue-available u1000000) total-tokens))
          (initial-context { asset-id: asset-id, total-tokens: total-tokens, revenue-per-token: revenue-per-token, current-block: stacks-block-height, success: true })
          (result (fold distribute-to-holder holders initial-context)))
      (asserts! (get success result) err-batch-failed)
      (map-set revenue-pool asset-id u0)
      (ok true)
    )
  )
)
